...
sys.path.append('/shared/workspaces/jprofant/GitHub/wespeaker')
...
def extract(config='conf/config.yaml', **kwargs):
    ...
    with torch.no_grad():
        with kaldiio.WriteHelper('ark,scp:' + embed_ark + "," +
                                 embed_scp) as writer:
            with open(os.path.join(os.path.dirname(embed_ark), 'segments'), 'w') as f:
                for _, batch in tqdm(enumerate(dataloader)):
                    assert len(batch['label']) == 2
                    start, end = batch['label']
                    ...
                    for i, utt in enumerate(utts):
                        ...
                        print(_, embed)
                        writer(utt, embed)
                        f.write(f'{utt} {utt[:utt.rfind("_")]} {start:.2f} {end:.2f}\n')
